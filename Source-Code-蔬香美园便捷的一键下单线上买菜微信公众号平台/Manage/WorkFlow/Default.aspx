...
<ol id="BreadNav" class="breadcrumb navbar-fixed-top">
            ...
            <div class="pull-right mr-2 ml-auto"><a href="javascript:;" onclick="selbox.toggle();" id="sel_btn" class="help_btn"><i class="fa fa-search"></i></a></div>
    ...
    EmptyDataText="当前没有信息!!" OnRowDataBound="EGV_RowDataBound" DataKeyNames="ID" OnPageIndexChanging="EGV_PageIndexChanging" OnRowCommand="EGV_RowCommand">
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作"  ItemStyle-CssClass="max20rem">
                <ItemTemplate>
                    <a href="WorkFlow.aspx?proID=<%#Eval("ID") %>"><i class="fa fa-pencil" title="编辑"></i></a>
                    <asp:LinkButton ID="LinkButton1" runat="server" CommandName="Del2" CommandArgument='<%#Eval("ID") %>' OnClientClick="return confirm('你确定要删除吗!');" ToolTip="删除" ><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton>
                    <a href="AddFlow.aspx?proID=<%#Eval("ID") %>"><i class="fa fa-eye" title="查看"></i>查看步骤</a>
                    <a href="javascript:;" onclick="ShowImgDiag(<%#Eval("ID") %>)" ><i class="fa fa-eye" title="查看"></i>查看流程图</a>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>

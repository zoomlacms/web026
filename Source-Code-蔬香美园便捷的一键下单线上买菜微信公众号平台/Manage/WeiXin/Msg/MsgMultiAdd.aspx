<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MsgMultiAdd.aspx.cs" Inherits="ZoomLaCMS.Manage.WeiXin.Msg.MsgMultiAdd" MasterPageFile="~/Manage/I/Index.Master" ValidateRequest="false" %>
...
<%=Call.SetBread(new Bread[] {
        ...
        new Bread() {url="", text="多图文消息",addon="" }}
		)
    %>
	<div class="container-fluid">
	<div class="list_choice row table-responsive">
   ...
   <%--     <button type="button" class="btn btn-outline-info" onclick="PreWxMsg()">预览效果</button>--%>
        <asp:Button runat="server" ID="Save_Btn" CssClass="btn btn-info" Text="保存消息" OnClick="Save_Btn_Click" OnClientClick="return PreSave();" />
        <button type="button" class="btn btn-outline-info" onclick="backToList();">返回列表</button>
<%--        <asp:Button runat="server" ID="SaveNew_B" Visible="false" CssClass=btn btn-outline-info" Text="保存素材" OnClick="SaveNew_B_Click" OnClientClick="return PreSave();" />
        <asp:Button runat="server" ID="Update_Btn" CssClass="btn btn-info" Visible="false" Text="保存素材" OnClientClick="return UpdateData()" OnClick="Update_Btn_Click" />--%>
		</div>

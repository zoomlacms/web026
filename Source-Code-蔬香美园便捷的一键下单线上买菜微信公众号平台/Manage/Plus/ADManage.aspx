    ...
    <ZL:ExGridView ID="Egv" runat="server" AutoGenerateColumns="False" DataKeyNames="ADID" PageSize="20" 
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作" ItemStyle-CssClass="">
                <ItemTemplate>
                    <a href="Advertisement.aspx?ADId=<%# Eval("ADID") %>"><i class="fa fa-pencil" title="修改"></i></a>  
                    <asp:LinkButton runat="server" CommandName="Del" OnClientClick="return confirm('确定要删除此广告?');" CommandArgument='<%# Eval("ADID") %>' ><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton> 
                    <asp:LinkButton runat="server" CommandName="Copy" CommandArgument='<%# Eval("ADID") %>' ><i class="fa fa-copy" title="复制"></i>复制</asp:LinkButton>
                    <asp:LinkButton runat="server" CommandName="Pass" CommandArgument='<%# Eval("ADID") %>'><i class="fa fa-legal" title="审核"></i><%# (bool)Eval("Passed") == false ? "通过审核" : "取消审核"%></asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>

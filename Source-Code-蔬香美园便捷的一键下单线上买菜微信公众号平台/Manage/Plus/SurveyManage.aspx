<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SurveyManage.aspx.cs" Inherits="ZoomLaCMS.Manage.Plus.SurveyManage" MasterPageFile="~/Manage/I/Index.master" %>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    <div id="BreadDiv" class="container-fluid mysite">
        <div class="row">
            <ol id="BreadNav" class="breadcrumb navbar-fixed-top w-100">
                ...
                <li class="breadcrumb-item"><a href="SurveyManage.aspx">问卷投票管理</a><a href="Survey.aspx">[添加问卷投票]</a></li>
    ...
    <ZL:ExGridView ID="EGV"  CssClass="table table-striped table-bordered margin_t10" GridLines="None" runat="server" AutoGenerateColumns="False" AllowPaging="True" PageSize="10"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="类别" ItemStyle-CssClass="">
            ...
            <asp:TemplateField HeaderText="操作" ItemStyle-Width="350">
                <ItemTemplate>
                    <a href="/Vote/Index?id=<%#Eval("ID") %>" target="_blank" ><i class="fa fa-eye"></i>查看</a>
                    <a href="SurveyItemList.aspx?Sid=<%#Eval("ID") %>" ><i class="fa fa-list-alt"></i>问题列表</a>
                    <a href="SurveyResult.aspx?id=<%#Eval("ID") %>" ><i class="fa fa-pie-chart" title="分析"></i>结果分析</a>
                    <a href="SurveyResultList.aspx?ID=<%#Eval("ID") %>" ><i class="fa fa-list"></i>结果列表</a>
                    <a href="Survey.aspx?ID=<%# Eval("ID") %>"><i class="fa fa-pencil" title="修改"></i></a>
                    <asp:LinkButton ID="lbtnDel" runat="server" CommandName="Del" CommandArgument='<%# Eval("ID") %>' OnClientClick="return confirm('确实要删除吗？');" ><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>
    <div>
        <asp:Button runat="server" ID="BatStop_Btn" Text="批量停用" OnClick="BatCmd_Btn_Click" CommandName="stop" class="btn btn-outline-danger"/>
        <asp:Button runat="server" ID="BatStart_Btn" Text="批量启用" OnClick="BatCmd_Btn_Click" CommandName="start" class="btn btn-outline-info"/>
        <asp:Button runat="server" ID="BatDel_Btn" Text="批量删除" OnClick="BatCmd_Btn_Click" CommandName="del" class="btn btn-outline-danger" OnClientClick="return confirm('确定要删除吗');" />
    </div>
</asp:Content>

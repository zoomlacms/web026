...
<asp:Content  ContentPlaceHolderID="Content" Runat="Server">
    ...
    <div class="template">
        ...
        <Columns>
		    ...
		    <ItemTemplate>                 
                <asp:LinkButton ID="LinkButton4" runat="server" CommandName="Reply" CommandArgument='<%# Eval("GID")+"&CateID="+Eval("CateID") %>'  ToolTip="回复"><i class="fa fa-reply"></i>回复</asp:LinkButton>
                <asp:LinkButton ID="LinkButton1" runat="server" CommandName="RList" CommandArgument='<%# Eval("GID")+"&CateID="+Eval("CateID") %>'  ToolTip="回复列表"><i class="fa fa-list"></i>回复列表</asp:LinkButton>
                ...
                <a href="/Guest/GuestShow?GID=<%#Eval("GID") %>"  target="_blank" ><i class="fa fa-globe"></i>浏览</a>
                <asp:LinkButton ID="LinkButton5" runat="server" Visible='<%#Eval("Status","").Equals("0") %>' CommandName="Audit" CommandArgument='<%#Eval("GID")%>'  ToolTip="未审核"><i class="fa fa-flag"></i><span class="text-success">通过审核</span></asp:LinkButton>
                <asp:LinkButton ID="LinkButton6" runat="server" Visible='<%#Eval("Status","").Equals("1") %>' CommandName="CancelAudit" CommandArgument='<%#Eval("GID")%>'  ToolTip="已审核"><i class="fa fa-flag"></i><span class="text-danger">取消审核</span></asp:LinkButton>
                <asp:LinkButton ID="LinkButton2" runat="server" Visible='<%#!Eval("Status","").Equals("-1") %>' CommandName="Del" CommandArgument='<%#Eval("GID")%>'  ToolTip="删除"><i class="fa fa-trash-o"></i>删除</asp:LinkButton>
                <asp:LinkButton ID="LinkButton8" runat="server" Visible='<%#Eval("Status","").Equals("-1") %>' CommandName="Del2" CommandArgument='<%#Eval("GID")%>'  ToolTip="删除"><i class="fa fa-trash-o"></i>彻底删除</asp:LinkButton>
		    </ItemTemplate>
		    <HeaderStyle />
		    </asp:TemplateField>
	    </Columns>

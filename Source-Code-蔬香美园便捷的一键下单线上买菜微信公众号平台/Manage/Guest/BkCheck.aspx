...
<asp:Content ContentPlaceHolderID="Content" runat="Server">
    <ol class="breadcrumb navbar-fixed-top">
        ...
        <div id="help" class="pull-right text-center mr-2" ><a href="javascript:;" onclick="selbox.toggle();" id="sel_btn" class="help_btn"><i class="fa fa-search"></i></a></div>
    ...
    <div class="panel panel-default">
        <div class="panel panel-body">
            <ZL:ExGridView ID="Egv" runat="server" AutoGenerateColumns="False" DataKeyNames="ID" PageSize="10" OnRowDataBound="Egv_RowDataBound" OnPageIndexChanging="Egv_PageIndexChanging" IsHoldState="false" OnRowCommand="Lnk_Click" AllowPaging="True" AllowSorting="True" CssClass="table table-striped table-bordered table-hover" EnableTheming="False" EnableModelValidation="True" EmptyDataText="无相关数据">
                <Columns>
                    <asp:TemplateField HeaderStyle-CssClass="max20rem">
                        <ItemTemplate>
          ...
          <%--          <asp:TemplateField HeaderText="分类" HeaderStyle-CssClass="max20rem">
                    ...
                    <asp:TemplateField HeaderText="操作">
                        <ItemTemplate>
                            <a href="/Baike/Details?id=<%#Eval("ID") %>" target="_blank" ><i class="fa fa-eye" title="预览"></i></a>
                            <a href="/Baike/BKEditor?id=<%#Eval("ID") %>&mode=admin" target="_blank" ><i class="fa fa-pencil" title="修改"></i></a>
                            <asp:LinkButton ID="LinkButton2" runat="server" OnClientClick="return confirm('确实要删除吗？');" CommandArgument='<%#Eval("ID") %>' CommandName="Del" CausesValidation="false" ><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton>
                            <%--<a href="baikeMerger?tittle=<%#Eval("Tittle").ToString()%><i class="fa fa-columns" title="合并"></i>合并词条</a>--%>
<%--                            <a href="InfoList?id=<%#Eval("ID").ToString()%><i class="fa fa-list-alt" title="列表"></i>信息列表</a>--%>
                              <a href="javascript:;" onclick="ShowBKList('<%#Eval("Flow") %>');"><i class="fa fa-list"></i>版本管理</a>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
            </ZL:ExGridView>
        ...
        <div class="panel panel-footer">
            <asp:Button ID="Button1" runat="server" Text="批量删除" OnClick="BtnSubmit1_Click" UseSubmitBehavior="False" OnClientClick="if(!confirm('确定要批量删除词条吗？')){return false;}" CssClass="btn btn-outline-info" />
            <asp:Button ID="Button2" runat="server" Text="批量审核" OnClick="BtnSubmit2_Click" UseSubmitBehavior="False" CssClass="btn btn-outline-info" />
            <asp:Button ID="Button3" runat="server" Text="取消审核" OnClick="BtnSubmit3_Click" UseSubmitBehavior="False" CssClass="btn btn-outline-info" />
            <asp:Button ID="Button4" runat="server" Text="推荐" OnClick="BtnSubmit4_Click" UseSubmitBehavior="False" CssClass="btn btn-outline-info" />
            <asp:Button ID="Button5" runat="server" Text="取消推荐" OnClick="BtnSubmit5_Click" UseSubmitBehavior="False" CssClass="btn btn-outline-info" />
            <asp:Button ID="Button8" runat="server" Text="特色" OnClick="BtnSubmit8_Click" UseSubmitBehavior="False" CssClass="btn btn-outline-info" />
            <asp:Button ID="Button9" runat="server" Text="取消特色" OnClick="BtnSubmit9_Click" UseSubmitBehavior="False" CssClass="btn btn-outline-info" />
            <asp:Button ID="Button6" runat="server" Text="每日图片" OnClick="BtnSubmit6_Click" UseSubmitBehavior="False" CssClass="btn btn-outline-info" />
            <asp:Button ID="Button7" runat="server" Text="取消每日图片" OnClick="BtnSubmit7_Click" UseSubmitBehavior="False" CssClass="btn btn-outline-info" />
        </div>
    </div>
</asp:Content>

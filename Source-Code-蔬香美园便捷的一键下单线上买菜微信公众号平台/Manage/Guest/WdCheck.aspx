            ...
            <ZL:ExGridView ID="Egv" runat="server" AutoGenerateColumns="False" DataKeyNames="ID" PageSize="10" OnPageIndexChanging="Egv_PageIndexChanging" IsHoldState="false" OnRowCommand="Lnk_Click" AllowPaging="True" AllowSorting="True" CssClass="table table-striped table-bordered table-hover" EnableTheming="False" EnableModelValidation="True" EmptyDataText="无相关数据">
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="操作">
                        <ItemTemplate>
                            <a href="/Ask/SearchDetails?soure=manager&ID=<%#Eval("ID") %>" target="_blank"><i class="fa fa-eye" title="预览"></i></a>
                            <a href="WdAlter.aspx?ID=<%#Eval("ID")%>" target="_self"><i class="fa fa-pencil" title="修改"></i></a>
                            <asp:LinkButton ID="LinkButton2" runat="server" OnClientClick="return confirm('确实要删除吗？');" CommandArgument='<%#Eval("ID") %>'
                                CommandName="Del" CausesValidation="false"><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton>
                            <a href="Asklist.aspx?ID=<%#Eval("ID")%>"><i class="fa fa-comment-o" title="留言"></i>留言</a>
                            <asp:LinkButton ID="LinkButton1" runat="server" CommandArgument='<%#Eval("ID") %>' CommandName="Audit" CausesValidation="false" ><i class="fa fa-legal" title="审核"></i>审核</asp:LinkButton>
                            <a href="javascript:;" onclick="SelUser(<%#Eval("ID") %>)"><i class="fa fa-send" title="推送"></i>推送</a>
                        </ItemTemplate>

...
<asp:Content  ContentPlaceHolderID="Content" Runat="Server">
	<%=Call.SetBread(new Bread[] {
        ...
        new Bread() {url="", text="<a href=''>[添加栏目]</a><a href=''>[排序管理]</a><a href=''>[回收站]</a>",addon="" }},
		Call.GetHelp(92)
		)
    %>
    <asp:Repeater runat="server" ID="RPT">
        <HeaderTemplate>
            <table id="EGV"  class="table table-bordered table-hover table-striped list_choice listEGV">
        ...
        <ItemTemplate>
            <tr data-layer="1" data-id="<%#Eval("CateID") %>" data-pid="0"  onclick="ShowChild(this,<%#Eval("CateID") %>);" ondblclick="ToggleChild(this);">
                ...
                <td><a href="TieList.aspx?CateID=<%#Eval("CateID") %>"><span data-type="icon" class="<%#Convert.ToInt32(Eval("ChildCount"))>0?"fa fa-folder rep_tree_icon":"fa fa-file rep_tree_icon" %>" /></span></a><%#GetCateName()%></td>
                ...
                <td class='optd'>
                    <a href="javascript:;" onclick="ShowCate(<%#Eval("CateID") %>,0);"><i class="fa fa-pencil" title="修改"></i></a>
                    <a href="javascript:;" onclick="ShowCate(0,<%#Eval("CateID")%>);"><i class='fa fa-plus' title='添加'></i>版面</a>
                    <a href='BarAuthSet.aspx?ID=<%#Eval("CateID")%>'><i class='fa fa-key' title='权限'></i>权限</a>
                    <%#Convert.ToInt32(Eval("ChildCount"))>0?"<a href=\"javascript:ShowOrder('"+Eval("Cateid")+"');\"><i class='fa fa-list-ol' title='排序'></i>排序</a>":"" %>
                    <a href="javascript:;" onclick="DelConfirm(<%#Eval("CateID") %>,1)"><i class="fa fa-file-o" title="清空"></i>清空本版数据</a>
                    <a href="javascript:;" onclick='DelConfirm(<%#Eval("CateID") %>,0);'><i class="fa fa-trash-o" title="删除"></i>删除</a>
                </td>
            </tr>
        </ItemTemplate>
        <FooterTemplate>
    ...
    <script type="text/javascript">
        ...
        + "<td >@LayerHtml<img src='/Images/TreeLineImages/t.gif'><a href='TieList.aspx?CateID=@Cateid'><span data-type='icon' class='@hasChImg'></span></a><a href='javascript:;' onclick='ShowCate(@Cateid,0)'>@Catename</a></td>"
        + "<td><span class='text-primary'><span class='text-primary'>@BarInfo</span></span></td><td>@NeedLogStr</td><td>@CountStr</td>"
        + "<td class='optd'><a href='javascript:;' onclick='ShowCate(@Cateid,0);'><i class='fa fa-pencil' title='修改'></i></a>"
        + "<a href='javascript:;' onclick='ShowCate(0,@Cateid);'><i class='fa fa-plus' title='添加'></i>版面</a>"
        ...
        function ReplaceTlp(tlp, layer, data) {
            ...
            for (i = 0; i < data.length; i++) {
                html += tlp.replace(/@Cateid/g, data[i].Cateid).replace(/@Catename/g, data[i].Catename).replace(/@CatePid/, data[i].ParentID)
                    ...
                    .replace(/@hasChOrder/, parseInt(data[i].ChildCount) > 0 ? "<a href=\"javascript:open_page('SetCateOrder.aspx?id=" + data[i].Cateid + "');><i class='fa fa-list-ol' title='排序'></i>排序</a>" : "");
            }
            return html;
        }
        //确定是要显示还是隐藏

    ...
    <asp:Repeater runat="server" ID="RPT" OnItemCommand="RPT_ItemCommand">
        ...
        <ItemTemplate>
            <tr data-layer="1" data-id="<%#Eval("CateID") %>" data-pid="0" ondblclick="ShowCate(<%#Eval("CateID") %>,0);">
                ...
                <td class='optd'>
                    <a href="javascript:;"  onclick="ShowCate(<%#Eval("CateID") %>,0);"><i class="fa fa-pencil" title="修改"></i>修改 </a> 
                    <a href="Default.aspx?CateID=<%#Eval("CateID") %>"><i class="fa fa-list-ul"></i>留言列表 </a>
                    <a href="/Guest/Default?CateID=<%#Eval("CateID") %>" target="_blank"><i class="fa fa-globe"></i>浏览</a>
                    <asp:LinkButton CommandName="Del"  CommandArgument='<%#Eval("CateID") %>' OnClientClick="return confirm('确实要删除吗？');" runat="server"><i class="fa fa-trash-o" title="删除"></i>删除</asp:LinkButton>
                </td>
            </tr>
        </ItemTemplate>
        <FooterTemplate>

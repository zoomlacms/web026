...
new Bread("/{manage}/Pub/FormManage.aspx","互动表单管理"),
new Bread() {url="FormDesign.aspx", text="添加互动表单",addon="" }}
)
%>
</asp:Content>
      ...
      <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="10"  EnableTheming="False" 
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="<%$Resources:L,操作 %>">
                        <ItemTemplate>
                            <asp:LinkButton runat="server" CommandName="Del2" CommandArgument='<%#Eval("PubID") %>' OnClientClick="return confirm('你确定要删除吗!');" ToolTip="<%$Resources:L,删除 %>" ><i class="fa fa-trash-o" title="<%=Resources.L.删除 %>"></i></asp:LinkButton>
                            <a href="Pubinfo.aspx?menu=edit&ID=<%#Eval("PubID") %>" title="<%=Resources.L.修改配置 %>"><i class="fa fa-gear" title="<%=Resources.L.配置 %>"></i><%=Resources.L.配置 %></a>
                            <a href="FormDesign.aspx?ID=<%#Eval("PubID") %>" target="_blank" title="<%=Resources.L.设计表单 %>"><i class="fa fa-pencil-square-o" title="<%=Resources.L.设计 %>"></i><%=Resources.L.设计 %></a>
                            <a href="/Rss/FormView?Pid=<%#Eval("PubID") %>" target="_blank" title="<%=Resources.L.前台浏览 %>" ><i class='fa fa-globe'></i><%=Resources.L.浏览 %></a>
                            <a href="FormResult.aspx?ID=<%#Eval("PubID") %>" title="<%=Resources.L.查看统计 %>" ><i class="fa fa-pie-chart" title="<%=Resources.L.统计 %>"></i><%=Resources.L.统计 %></a>
                        </ItemTemplate>
                    </asp:TemplateField>
                   </Columns>

...
<asp:Content ID="Content2" ContentPlaceHolderID="Content" Runat="Server">
    <div class="container-fluid p-0 pr-2">
        <ZL:ExGridView ID="Egv" runat="server" AutoGenerateColumns="False" 
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="<%$Resources:L,操作 %>">
                    <ItemTemplate>
                        <a href="pubinfo.aspx?menu=edit&id=<%#Eval("Pubid")%>" > <i class="fa fa-pencil" title="<%=Resources.L.修改 %>"></i><%=Resources.L.修改 %></a>
                        <a href="Pubsinfo.aspx?Pubid=<%#Eval("PubID")%>&type=0" > <i class="fa fa-magic" title="信息列表"></i>信息列表</a>
                        <a href="pubinfo.aspx?menu=copy&id=<%#Eval("Pubid")%>" onclick="return confirm('<%=Resources.L.确实要复制吗 %>?');" > <i class="fa fa-copy" title="<%=Resources.L.复制 %>"></i><%=Resources.L.复制 %></a>
                        <a href="pubinfo.aspx?menu=delete&id=<%#Eval("Pubid")%>" onclick="return confirm('<%=Resources.L.确实要放入存档吗 %>?<%=Resources.L.确实要放入存档吗 %>!');" ><i class="fa fa-save" title="<%=Resources.L.存档 %>"> </i><%=Resources.L.存档 %></a>
                        <asp:LinkButton runat="server"  OnClientClick="return confirm('是否删除该项!')" CommandName="Del" CommandArgument='<%#Eval("PubID") %>'><i class="fa fa-trash-o"></i><%=Resources.L.删除 %></asp:LinkButton>
                    </ItemTemplate>
                </asp:TemplateField>
            </Columns>

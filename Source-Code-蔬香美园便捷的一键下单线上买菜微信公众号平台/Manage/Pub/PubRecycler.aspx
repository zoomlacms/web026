    ...
    <div class="panel panel-default pr-2 list_choice">
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="<%$Resources:L,操作 %>">
                <ItemStyle Width="15%" />
                <ItemTemplate>
                    <a href="pubinfo.aspx?menu=edit&id=<%#Eval("Pubid")%>" ><i class="fa fa-pencil" title="<%=Resources.L.修改 %>"></i></a>
                    <a href="pubinfo.aspx?menu=Truedelete&id=<%#Eval("Pubid")%>" onclick="return confirm('确实要彻底删除吗？');" ><i class="fa fa-trash" title="<%=Resources.L.彻底删除 %>"></i></a>
                    <a href="pubinfo.aspx?menu=revert&id=<%#Eval("Pubid")%>" ><i class="fa fa-recycle" title="<%=Resources.L.还原 %>"></i><%=Resources.L.还原 %></a>
                    <a href="Pubsinfo.aspx?Pubid=<%#Eval("Pubid") %>&type=0" ><i class="fa fa-magic" title="<%=Resources.L.管理 %>"></i><%=Resources.L.管理信息 %></a>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>

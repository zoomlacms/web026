...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    <ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="false" AllowPaging="true" PageSize="10" EnableTheming="False" CssClass="table table-striped table-bordered table-hover" 
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                <%--    <a href="EditOrderSql.aspx?ID=<%#Eval("id") %>" ><i class="fa fa-eye" title="查看"></i></a>--%>
              <%--      <asp:LinkButton ID="StatBtn"  runat="server" Text='<%#getStabtn(Eval("status","{0}"))%>' ><span class="text-danger"></span></asp:LinkButton>
                    ...
                    <asp:LinkButton ID="Del_btn" runat="server" CommandName='Del' Text='删除' CommandArgument='<%#Eval("ID") %>' ><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton>
                   <%-- <asp:LinkButton ID="runBtn" runat="server" Text='执行' CommandName="Run" CommandArgument='<%#Eval("ID") %>'><i class="fa fa-cubes" title="批处理"></i> 执行批处理 </asp:LinkButton>--%>
                    <asp:LinkButton ID="LinkButton1" runat="server" Text='生成' CommandName="save" CommandArgument='<%#Eval("ID") %>' ><i class="fa fa-spinner" title="生成"></i>生成脚本文件</asp:LinkButton>
                    <asp:LinkButton ID="save_btn" runat="server" Text='下载' CommandName="download" CommandArgument='<%#Eval("ID") %>' ><i class="fa fa-download" title="下载"></i>下载数据库脚本</asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>
    ...
    <asp:Button runat="server" ID="BatUnAudit_Btn" Text="取消审核" OnClick="BatUnAudit_Btn_Click" CssClass="btn btn-outline-info"/>

...
<%--            <asp:TemplateField HeaderText="<%$Resources:L,类型 %>">
            ...
            <asp:TemplateField HeaderText="<%$Resources:L,操作 %>">
                <ItemTemplate>
                    ...
                    <a  href="SuitProAdd.aspx?id=<%#Eval("id")%>"><i class="fa fa-pencil" title="<%=Resources.L.编辑 %>"></i></a>
                    <asp:LinkButton runat="server"  CommandName="del" CommandArgument='<%# Eval("id") %>' OnClientClick="return confirm('确定要将商品移入回收站吗');"><i class="fa fa-trash-o" title="<%=Resources.L.删除 %>"></i><%=Resources.L.删除 %></asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>

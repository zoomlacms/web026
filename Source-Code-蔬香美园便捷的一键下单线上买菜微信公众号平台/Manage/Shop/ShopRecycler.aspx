<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ShopRecycler.aspx.cs" Inherits="ZoomLaCMS.Manage.Shop.ShopRecycler" MasterPageFile="~/Manage/I/Index.master"%>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
     <ol id="BreadNav" class="breadcrumb navbar-fixed-top">
        ...
        <li class='breadcrumb-item'>商品回收站</li>
        ...
        <div id="sel_box" class="padding5">
            <div class="input-group">
                ...
                <span class="input-group-btn">
                    <asp:LinkButton runat="server" class="btn btn-outline-dark" ID="Search_Btn" OnClick="Search_Btn_Click"><i class="fa fa-search"></i></asp:LinkButton>
                </span>
            </div>
        </div>
    </ol>
    ...
    <div class="btn-group alert alert-light">
        <asp:Button ID="Rec_B" class="btn btn-outline-info" runat="server" Text="批量还原" OnClick="Rec_B_Click" OnClientClick="if(!IsSelectedId()){alert('请选择还原项');return false;}else{return true;}" />
        <asp:Button ID="Del_B" class="btn btn-outline-danger" runat="server" Text="批量删除" OnClick="Del_B_Click" OnClientClick="if(!IsSelectedId()){alert('请选择删除项');return false;}else{return confirm('不可恢复性删除数据,你确定将该数据删除吗？')}" />
        <asp:Button ID="DelAll_B" class="btn btn-outline-danger" runat="server" Text="清空回收站" OnClick="DelAll_B_Click" OnClientClick="return confirm('你确定要清空回收站吗')" />
    </div>
</asp:Content>

    ...
    <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" class="table table-striped table-bordered table-hover list_choice" EmptyDataText="您还没有打印设备"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="<%$Resources:L, 操作%>">
                <ItemTemplate>
                    <a  href="TestPrint.aspx?DevId=<%#Eval("ID") %>"><i class="fa fa-print" title="打印"></i>打印</a>
                    <a  href="AddDevice.aspx?id=<%#Eval("ID") %>"><i class="fa fa-pencil" title="修改"></i>修改</a>
                    <asp:LinkButton ID="SetDef_B" runat="server" CommandArgument='<%#Eval("ID") %>' CommandName="setDef" OnClientClick='<%# Eval("IsDefault").ToString().Equals("1") ? "return false;":"return true;" %>' ><span style='<%# Eval("IsDefault").ToString().Equals("1") ? "color:#b1b1b1;":"" %>'><i class="fa fa-check"></i>设为默认</span></asp:LinkButton>
                    <asp:LinkButton ID="Del" runat="server" CommandArgument='<%#Eval("ID") %>' CommandName="del" OnClientClick="return confirm('确认删除？')"><i class="fa fa-trash-o" title="<%=Resources.L.删除 %>"></i><%=Resources.L.删除 %></asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>

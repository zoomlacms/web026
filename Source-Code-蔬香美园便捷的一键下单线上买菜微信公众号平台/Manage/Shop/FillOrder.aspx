<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FillOrder.aspx.cs" Inherits="ZoomLaCMS.Manage.Shop.FillOrder"MasterPageFile="~/Manage/I/Index.master"%>
...
<div class="userlist" id="userlist" runat="server">
    <ZL:ExGridView ID="EGV"  runat="server" AutoGenerateColumns="False" PageSize="20"  OnRowCommand="EGV_RowCommand" OnPageIndexChanging="EGV_PageIndexChanging" 
        IsHoldState="false" Height="20px"   AllowPaging="True" AllowSorting="True" CssClass="table table-striped table-bordered table-hover list_choice " EnableTheming="False" 
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="<%$Resources:L,操作 %>">
                <ItemTemplate>
                    <asp:LinkButton ID="LinkButton1" CommandName="select" CommandArgument='<%#Eval("UserID") %>' runat="server"><i class="fa fa-plus" title="<%=Resources.L.选择会员 %>"></i><%=Resources.L.选择会员 %></asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>

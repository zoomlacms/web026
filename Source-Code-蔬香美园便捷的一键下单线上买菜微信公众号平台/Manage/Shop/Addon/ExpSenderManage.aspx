...
<li class="breadcrumb-item"><a href='Exp/DeliverType.aspx'>商城设置</a></li>
<li class='breadcrumb-item'>发件信息<a href='AddExpSender.aspx'>[添加发件信息]</a></li>
        ...
        <div id="sel_box" runat="server" class="padding5">
            ...
            <div class="input-group pull-left max20rem">
                ...
                <span class="input-group-btn">
                    <asp:LinkButton runat="server" CssClass="btn btn-outline-dar" ID="souchok" OnClick="souchok_Click"><span class="fa fa-search"></span></asp:LinkButton>
                </span>
            </div>
        </div>
    </ol>
    <ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="False" PageSize="10" OnPageIndexChanging="EGV_PageIndexChanging" IsHoldState="false" OnRowCommand="EGV_RowCommand" OnRowDataBound="EGV_RowDataBound"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                    <a href="AddExpSender.aspx?id=<%#Eval("ID") %>"><i class="fa fa-pencil"></i>修改</a>
                    <span class="<%#Eval("IsDefault","")=="1" ? "enabled" : "" %>">
                        <asp:LinkButton runat="server" CommandName="setdef" Enabled='<%#Eval("IsDefault","")!="1" %>' CommandArgument='<%#Eval("ID") %>'><i class="fa fa-check"></i>设为默认</asp:LinkButton></span>
                    <asp:LinkButton runat="server" CommandName="del" CommandArgument='<%#Eval("ID") %>' OnClientClick="return confirm('你确定将该数据删除吗？');"><i class="fa fa-trash-o"></i>删除</asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>

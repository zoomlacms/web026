     ...
     <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="20"  EnableTheming="False"  
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                    <a href="AddRole.aspx?RoleID=<%#Eval("RoleID") %>"  ><i class="fa fa-pencil" title="修改"></i></a>
                    <asp:LinkButton ID="LnkDel" CommandName="Del" CommandArgument='<%# Eval("RoleID")%>' OnClientClick="return confirm('确认删除该角色?')" runat="server"  Visible='<%#!Eval("RoleID","").Equals("1") %>'><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton>
                    <a href="../AdminManage.aspx?Rid=<%#Eval("RoleID") %>"   <%#Eval("RoleID","").Equals("1")?"hidden":"" %>"><i class="fa fa-users" title="成员管理"></i>成员管理</a>
                    <a href="RoleAuthList.aspx?id=<%#Eval("RoleID") %>"   <%#Eval("RoleID","").Equals("1")?"hidden":"" %>"><i class="fa fa-key" title="权限设置"></i>权限设置</a>
                </ItemTemplate>

<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PermissionInfo.aspx.cs" Inherits="ZoomLaCMS.Manage.User.PermissionInfo"  MasterPageFile="~/Manage/I/Index.master"%>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    <%=Call.SetBread(new Bread[] {
        ...
        new Bread() {url="", text="角色管理<a href='Permissionadd.aspx'>[添加角色]</a>",addon="" }}
		)
    %>
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="相关操作" ItemStyle-CssClass="td_l">
                <ItemTemplate>
                    ...
                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("ID") %>' CommandName="del2" OnClientClick="return confirm('确定删除?');"><i class="fa fa-trash" title="删除"></i> </asp:LinkButton>
                    <a href="CompetenceAdd.aspx?id=<%#Eval("ID") %>" ><i class="fa fa-key" title="权限设置"></i>权限</a>
                    <a href="SelUsersToRole.aspx?id=<%#Eval("ID") %>"><i class="fa fa-users" title="用户管理"></i>用户</a>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>
	<div class="alert alert-light">
    <asp:Button runat="server" ID="BatDel_Btn" CssClass="btn btn-outline-danger" Text="批量移除" OnClick="BatDel_Btn_Click" OnClientClick="return confirm('确定要移除吗?');" />
	</div>
</asp:Content>

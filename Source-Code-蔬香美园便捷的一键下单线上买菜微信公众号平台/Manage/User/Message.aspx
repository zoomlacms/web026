    ...
    <div id="sel_box" runat="server">
        <div>
			...
			<div  class="input-group-append">
				<asp:Button ID="Search_B" runat="server" Text="<%$Resources:L,搜索 %>" class="btn btn-outline-dark" OnClick="Search_B_Click" />
			</div>
			</div>
        </div>
    </div>
...
<div class="template" id="template" runat="server">
  ...
  <%--          <asp:TemplateField HeaderText="类型" ItemStyle-CssClass="max20rem">
            ...
            <asp:TemplateField HeaderText="操作" ItemStyle-CssClass="max20rem">
                <ItemTemplate>
                    <asp:LinkButton ID="btnDel" runat="server" CommandName="DeleteMsg" OnClientClick="if(!this.disabled) return confirm('确定删除此信息?');"  CommandArgument='<%# Eval("MsgID")%>' ><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton>
                    <a href="MessageRead.aspx?id=<%# Eval("MsgID")%>"><i class="fa fa-eye" title="阅读"></i>阅读</a>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView> 
...
<asp:Button runat="server" ID="BatDel_Btn" Text="批量删除" OnClick="BatDel_Btn_Click" class="btn btn-outline-info" />
</div>
</asp:Content>

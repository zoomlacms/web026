<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="UnitMain.aspx.cs" Inherits="ZoomLaCMS.Manage.User.UnitMain"  MasterPageFile="~/Manage/I/Index.master" %>
...
new Bread(){url="UnitMain.aspx", text="会员提成"}
}) %>
    <div class="d-flex flex-row table table-bordered list_choice">
        <ZL:StructDP runat="server" ID="StructDP" />
        <asp:TextBox runat="server" ID="Time_T" onclick="WdatePicker({dateFmt:'yyyy/MM/dd',firstDayOfWeek:1,isShowWeek:'true',onpicked:function(){WhichWeek();}});" CssClass="form-control max20rem" />
        <input type="text" id="Week_T" class="form-control w_6rem" readonly="readonly"/>
        <asp:Button runat="server" ID="Begin_Btn" Text="查询" OnClick="Begin_Btn_Click" CssClass="btn btn-outline-secondary" />
...
<%--        <asp:Button runat="server" ID="AutoUnit_Btn" Text="自动分成" OnClientClick="return UnitConfirm(this);" OnClick="AutoUnit_Btn_Click" CssClass="btn btn-danger" />--%>
    ...
    <div class="table table-bordered p-2">

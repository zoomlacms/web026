...
<asp:Content runat="server" ContentPlaceHolderID="Content">
	<%=Call.SetBread(new Bread[] {
        ...
        new Bread() {url="", text="会员导航<a href='AddUserFunc.aspx?LinkType=3'>[添加会员导航]</a>",addon="" }}
		)
    %>
    <ul class="nav nav-tabs list_choice" id="linktype_ul">
      <li class="nav-item" data-id="2" role="presentation" ><a class="nav-link" href="UserFunc.aspx?EliteLevel=2"><%=Resources.L.所有 %></a></li> 
      <li class="nav-item" data-id="1" role="presentation"><a class="nav-link" href="UserFunc.aspx?EliteLevel=1"><%=Resources.L.推荐 %></a></li>
      <li class="nav-item" data-id="0" role="presentation"><a class="nav-link" href="UserFunc.aspx"><%=Resources.L.普通 %></a></li> 
    </ul>
    <div>
        <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="False" PageSize="20" AllowPaging="true" OnPageIndexChanging="EGV_PageIndexChanging"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="会员组" ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle">
                    <ItemTemplate>
                        <%#GetUserGroup() %>
                    </ItemTemplate>
                </asp:TemplateField>
        ...
        <div class="nav-item nav-link text-center">
        <asp:Button ID="BtnDelete" runat="server" CssClass="btn btn-outline-danger" OnClick="BtnDelete_Click" OnClientClick="if(!IsSelectedId()){alert('请选择内容');return false;}else{return confirm('确定删除？')}" Text="<%$Resources:L,删除导航 %>"/>
        <asp:Button ID="Button1" runat="server" Text="<%$Resources:L,批量启用 %>" CssClass="btn btn-outline-info" OnClick="Btnuse_Click" />
        <asp:Button ID="Button2" runat="server" Text="<%$Resources:L,批量停用 %>" CssClass="btn btn-outline-danger" OnClick="Btnstop_Click" />
        <asp:Button ID="Button3" runat="server" Text="<%$Resources:L,保存排序 %>" CssClass="btn btn-outline-info" OnClick="saveOrder_Btn_Click" OnClientClick="return confirm('确定要保存修改后的排序吗!!!');"/>
    </div>
    </div>
</asp:Content>

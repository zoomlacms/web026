<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DatalistProfile.aspx.cs" Inherits="ZoomLaCMS.Manage.Config.DatalistProfile" MasterPageFile="~/Manage/I/Index.master"%>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    ...
    <div id="Empty_Div" runat="server" visible="false" class="alert alert-info" style="margin-top:5px;">
        当前数据库中没有数据库标注信息，可从云台获取脚本初始化数据库标注数据，点击
        <asp:LinkButton ID="DownData_B" runat="server"  OnClick="DownData_B_Click"><i class="fa fa-download"></i>下载</asp:LinkButton>
    </div>
   <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="20" EnableTheming="False"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="操作" ItemStyle-CssClass="">
                    <ItemTemplate>
                        <a href="EditDataList.aspx?ID=<%#Eval("ID") %>" ><i class="fa fa-pencil" title="修改"></i></a>
                        <a href="DBTools/Default.aspx" ><i class="fa fa-table" title="表结构"></i>表结构</a>
                        <a href="DataListView.aspx?ID=<%#Eval("ID") %>" ><i class="fa fa-reorder" title="表数据"></i>表数据</a>
                    </ItemTemplate>
                </asp:TemplateField>
            </Columns>
        </ZL:ExGridView>

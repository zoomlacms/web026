	...
	<Columns>
       ...
       <asp:TemplateField HeaderText="流程名称" ItemStyle-CssClass="text-left">
		...
		<asp:TemplateField HeaderText="流程操作" ShowHeader="False" ItemStyle-CssClass="td_l">
			<ItemTemplate>
                <a href='AddFlow.aspx?id=<%#Eval("id") %>' ><i class="fa fa-pencil" title="修改"></i></a>
                <asp:LinkButton ID="lbDel" runat="server" CausesValidation="False" CommandName="Delete"  OnClientClick="return confirm('确定要删除吗');"><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton>
                <a href='FlowManager.aspx?Action=copy&id=<%#Eval("id") %>' ><i class="fa fa-copy" title="复制"></i></a>
				<a href='FlowProcess.aspx?flowid=<%# Eval("id") %>'><i class="fa fa-plus" title="添加"></i>步骤</a>
                <a href='FlowProcessManager.aspx?id=<%# Eval("id") %>'><i class="fa fa-list-alt" title="列表"></i>列表</a>
			</ItemTemplate>
		</asp:TemplateField>
	</Columns>
</ZL:ExGridView>

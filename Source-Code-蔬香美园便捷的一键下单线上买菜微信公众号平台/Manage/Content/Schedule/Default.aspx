<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="ZoomLaCMS.Manage.Content.Schedule.Default" MasterPageFile="~/Manage/I/Index.master" %>
...
<ol id="BreadNav" class="breadcrumb navbar-fixed-top">
    ...
    <li class="breadcrumb-item"><a href="Default.aspx">任务列表</a>[<a href="AddSche.aspx">创建任务</a>]</li>
...
<ul class="nav nav-tabs">
    <li class="nav-item" data-type="1" ><a class="nav-link active" href="Default.aspx?TaskType=1">全部任务</a></li>
...
<ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="False" PageSize="10" IsHoldState="false" 
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                    <a  href="AddSche.aspx?id=<%#Eval("ID") %>"><i class="fa fa-pencil" title="修改"></i></a>
                    ...
                    <a  href="ScheLogList.aspx?TaskID=<%#Eval("ID") %>"><i class="fa fa-file-text"></i>日志</a>
                    <asp:LinkButton runat="server"  CommandName="execNow" CommandArgument='<%#Eval("ID") %>' OnClientClick="return confirm('将强行执行（忽略系统计划和预先规则），是否继续？');"><i class="fa fa-hourglass-start"></i>强制执行</asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>
<div>
    <asp:Button runat="server" ID="StartTask_Btn" Text="启动任务" CssClass="btn btn-outline-info" OnClick="StartTask_Btn_Click" OnClientClick="return confirm('确定要将已停用的任务加入队列吗');" />
    <asp:Button runat="server" ID="StopTask_Btn" Text="停用任务" CssClass="btn btn-outline-info" OnClick="StopTask_Btn_Click" OnClientClick="return confirm('确定要停用任务吗');" />
    <asp:Button runat="server" ID="InitTask_Btn" Text="初始化任务" class="btn btn-outline-info" OnClick="InitTask_Btn_Click" OnClientClick="return confirm('初始化会清空队列并重新生成任务,确定要初始化吗');" />
    <a href='TaskList.aspx' class="btn btn-outline-info"><i class="fa fa-eye"></i> 查看运行中的任务队列</a>
</div>
</div>
</asp:Content>

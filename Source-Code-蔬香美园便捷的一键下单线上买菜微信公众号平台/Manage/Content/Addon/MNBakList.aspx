...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    <div id="BreadDiv" class="container-fluid mysite">
        <div class="row">
            <ol id="BreadNav" class="breadcrumb navbar-fixed-top w-100">
                ...
                <li class="breadcrumb-item"><a href="/Admin/Config/DatalistProfile.aspx">扩展功能</a></li>
                <li class="breadcrumb-item">开发中心</li>
                <li class="breadcrumb-item">备份列表
                    <asp:LinkButton runat="server" ID="CreateBak_Btn" class="ml-2" OnClientClick="return confirm('确定要创建当前备份吗?');" OnClick="CreateBak_Btn_Click">[创建备份]</asp:LinkButton></li>
            </ol>
        </div>
    </div>
    ...
    <ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="false" PageSize="10" IsHoldState="false"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("name") %>' CommandName="download"  ToolTip="下载"><i class="fa fa-cloud-download"> </i></asp:LinkButton>
                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("name") %>' CommandName="restore"  OnClientClick="return confirm('[确定恢复该备份吗?]改为[系统将还原所有节点模型数据(此操作不可逆),是否继续?] 恢复前清空数据');"><i class="fa fa-recycle"> 数据恢复</i></asp:LinkButton>
                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("name") %>' CommandName="del2"  OnClientClick="return confirm('确定要删除该备份吗?');"><i class="fa  fa-trash-o"> 删除备份</i></asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>
    ...
    <div class="alert alert-danger">
        <ol>
            <li><span class="w-100">元数据还原后,系统的[ZL_Node,ZL_Node_ModelTemplate,ZL_Model,ZL_ModelField,ZL_UserPromotions,ZL_NodeRole,ZL_Pub]中的数据会进行还原</span></li>

...
<asp:Content ID="Content2" ContentPlaceHolderID="Content" runat="Server">
    ...
    <div class="list_choice">
                        ...
                        <ItemTemplate>
                            <div style="position: relative; cursor: pointer;">
                                ...
                                <div class="popover bottom" style="right: 0px; top: 20px; max-width: none;">
                                    <div class="arrow"></div>
                                    <h3 class="popover-title"><%#Eval("Title") %><span class="ml-2"><%# Convert.ToDateTime(Eval("CreateTime", "{0}")).ToShortDateString() %></span><span class="badge pull-right"><%# Eval("Hits") %></span><span class="pull-right">点击数：</span></h3>
        ...
        <div class="text-center">
            <asp:Button ID="btnClear" CssClass="btn btn-outline-danger" runat="server" Text="彻底删除选中" OnClick="btnClear_Click" OnClientClick="if(!IsSelectedId()){alert('请选择删除项');return false;}else{return confirm('数据删除后不可恢复，确认要删除选中项？')}" UseSubmitBehavior="true" />
            <asp:Button ID="btnRevert" CssClass="btn btn-outline-info" runat="server" Text="还原选中" OnClick="btnRevert_Click" OnClientClick="if(!IsSelectedId()){alert('请选择内容');return false;}else{return confirm('还原后请进入内容对信息进行审核！')}" />
            <asp:Button ID="btnRevertAll" CssClass="btn btn-outline-info" runat="server" Text="还原所有" OnClientClick="return confirm('还原后请进入内容对信息进行审核！')" OnClick="btnRevertAll_Click" />
            <asp:Button ID="btnClearAll" CssClass="btn btn-outline-danger" runat="server" Text="清空回收站" OnClick="btnClearAll_Click" OnClientClick="return confirm('数据删除后不可恢复，确认清空？')" />
        </div>

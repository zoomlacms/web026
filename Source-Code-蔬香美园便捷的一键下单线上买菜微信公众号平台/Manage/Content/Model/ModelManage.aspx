...
<%=Call.SetBread(new Bread[] {
        ...
        new Bread() {url="ModelManage.aspx?ModelType="+ModelType, text="内容模型管理<a href='AddEditModel.aspx?ModelType="+ ModelType+"'>[添加模型]</a>",addon="" }},
		Call.GetHelp(10)
		)
%>
</asp:Content>
...
<ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="False" DataKeyNames="ModelID" PageSize="20" OnRowDataBound="Egv_RowDataBound" 
        ...
        AllowPaging="True" AllowSorting="True" CssClass="table table-striped table-bordered table-hover sysRow list_choice" 
                ...
                <ItemTemplate>
					<div class="btn-group">
					  ...
					  <div class="dropdown-menu">
						<a href="ModelField.aspx?ModelID=<%#Eval("ModelID") %>&ModelType=<%:ModelType %>" class="dropdown-item"><i class="fa fa-align-justify"></i> 字段</a>
						<a href="AddEditModel.aspx?ModelID=<%#Eval("ModelID") %>&ModelType=<%:ModelType %>" class=" dropdown-item"><i class="fa fa-pencil"></i> <%=Resources.L.编辑 %></a>
						<asp:LinkButton runat="server" class="dropdown-item" CommandName="Copy" CommandArgument='<%# Eval("ModelID")%>' Enabled='<%#GetEnabled(Eval("SysModel").ToString()) %>'><i class="fa fa-paste"></i> <%=Resources.L.复制 %></asp:LinkButton>
						<asp:LinkButton runat="server" class="dropdown-item" CommandName="Del2" CommandArgument='<%# Eval("ModelID") %>' OnClientClick="return confirm('确实要删除此模型吗？');"><i class="fa fa-trash-o"></i> <%=Resources.L.删除 %></asp:LinkButton>
						<a href="ModelData.aspx?ID=<%#Eval("ModelID") %>" class="dropdown-item"><i class="fa fa-database"></i> 数据</a>
					  </div>
					</div>
                </ItemTemplate>
            </asp:TemplateField>

...
<%=Call.SetBread(new Bread[] {
        ...
        new Bread() {url="", text="版面管理 当前："+""+"<a href='Publish.aspx?Pid="+Pid+"'>[添加版面]</a>",addon="" }}
		)
    %>
    <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="10" EnableTheming="False"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                    <a href="/Rss/ViewPublish?Pid=<%#Eval("Pid") %>&ID=<%#Eval("ID") %>" target="_blank" title="前台浏览"><i class="fa fa-globe" title="浏览"></i></a><span class="sperspan"></span>
                    <a href="Publish.aspx?Pid=<%#Eval("Pid") %>&ID=<%#Eval("ID") %>" target="_parent"><i class="fa fa-pencil" title="修改"></i></a><span class="sperspan"></span>
                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("ID") %>' CommandName="Del2" OnClientClick="return confirm('确定要删除该版面?');" ><i class="fa fa-trash-o" title="删除"></i>删除</asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>

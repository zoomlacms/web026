...
<ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="False" PageSize="10" IsHoldState="false" 
    ...
    <Columns>
        ...
        <asp:TemplateField HeaderText="操作" ItemStyle-CssClass="">
            <ItemTemplate>
                <asp:LinkButton runat="server"  CommandName="stop" CommandArgument='<%#Eval("ID") %>' Visible='<%#Eval("ZStatus","").Equals("-1")?false:true %>'><i class="fa fa-ban"></i>停用</asp:LinkButton>
                <asp:LinkButton runat="server"  CommandName="active" CommandArgument='<%#Eval("ID") %>' Visible='<%#Eval("ZStatus","").Equals("-1")?true:false %>'><i class="fa fa-check"></i>启用</asp:LinkButton>
                <a  href="CSSContent.aspx?id=<%#Eval("ID") %>"><i class="fa fa-pencil" title="修改"></i></a>
                <asp:LinkButton runat="server"  CommandName="down" CommandArgument='<%#Eval("ID") %>'><i class="fa fa-download"></i></asp:LinkButton>

<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FtpAll.aspx.cs" Inherits="ZoomLaCMS.Manage.FtpFile.FtpAll" EnableViewStateMac="false" MasterPageFile="~/Manage/I/Index.master"%>
...
<%=Call.SetBread(new Bread[] {
        ...
        new Bread() {url="FtpAll.aspx", text="云端存储<a href='FtpConfig.aspx'>[添加云端服务器]</a>",addon="" }}
		)
    %>
    ...
    <asp:TemplateField HeaderText="操作">
      ...
      <ItemTemplate> 
          <a href='FtpConfig.aspx?DSId=<%# Eval("ID")%>'><i class="fa fa-pencil" title="修改"></i></a> 
          <a href="javascript:if(confirm('确定要删除?')) window.location.href='FtpAll.aspx?SId=<%# Eval("ID")%>';"><i class="fa fa-trash-o" title="删除"></i></a> 
          <%--<a href='ShowFtpFile.aspx?FId=<%# Eval("ID")%>'><i class="fa fa-magic" title="管理"></i>文件管理</a>--%>
      </ItemTemplate>
      <ItemStyle HorizontalAlign="Center" />
    </asp:TemplateField>
    </Columns>
  ...
  <table  class="table table-striped table-bordered table-hover" id="sleall">
    <tr>
      <td><asp:CheckBox ID="CheckBox1" runat="server" AutoPostBack="True" OnCheckedChanged="CheckBox1_CheckedChanged" Text="全选" />
        &nbsp; &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;
        <asp:Button ID="btndelete" class="btn btn-outline-danger" runat="server" OnClientClick="if(!IsSelectedId()){alert('请选择删除项');return false;}else{return confirm('你确定要将所有选择项删除吗？')}" Text="批量删除" OnClick="btndelete_Click" />
        &nbsp;
        </tr>
  </table>
 </div>

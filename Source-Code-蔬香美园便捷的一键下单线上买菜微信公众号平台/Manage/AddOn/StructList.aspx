...
<asp:Content ContentPlaceHolderID="Content" runat="Server"	>
	<%=Call.SetBread(new Bread[] {
        ...
        new Bread() {url="", text="根结构<a href='AddStruct.aspx?type=0'>[添加部门]</a>",addon="" }}
		)
    %>
    <div>
        ...
        <div class="alert alert-light">
            <asp:Button ID="Button1" class="btn btn-outline-danger" Text="批量排除" runat="server" OnClientClick="if(!IsSelectedId()){alert('请选择内容');return false;}else{return confirm('不可恢复性数据,你确定将该成员从此结构中排除吗？');}" OnClick="Button1_Click" />
            <input type="button" value="组织结构图" onclick="showStructPic();" class="btn btn-outline-info" />
        </div>
...
<asp:Content runat="server" ContentPlaceHolderID="ScriptContent">
    ...
    <script type="text/javascript">
        function showStructPic() {
            ShowComDiag("/JS/Plugs/OrgChart/StructTree.aspx", "组织结构图");
        }
